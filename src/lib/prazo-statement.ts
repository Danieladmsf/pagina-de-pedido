...
import { csvArquivo, csvCabecalhoDoCliente, csvDate, csvLine, csvNumber, nomeDeArquivo, type ExportCustomer } from '@/lib/csv';
...
// As primitivas de CSV (BOM, separador, número pt-BR, cabeçalho) moram em
// lib/csv.ts: a ficha do cliente exporta pelas MESMAS regras, e copiá-las seria
// repetir as decisões que fazem o arquivo abrir certo no Excel em português.

export type { ExportCustomer } from '@/lib/csv';
...
export function buildStatementCsv(rows: StatementRow[], customer: ExportCustomer): string {
  ...
  const linhas = [
    ...csvCabecalhoDoCliente(customer, 'Extrato do Prazo'),
...
export function buildItemsCsv(rows: StatementRow[], customer: ExportCustomer): string {
  const linhas = [
    ...csvCabecalhoDoCliente(customer, 'Itens comprados a prazo'),
...
export const exportFileName = nomeDeArquivo;
