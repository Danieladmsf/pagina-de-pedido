...
import { getWuzProfilePicture } from '@/lib/wuzapi/wuzapi.service';
...
/**
 * Foto de perfil de um contato (proxy seguro do servidor de WhatsApp). Recebe { empresaId, phone }.
...
export async function POST(request: Request) {
  return withAuth(request, async (user) => {
    try {
      ...
      const data = await getWuzProfilePicture(integration.wapiInstanceId, token, phone);
