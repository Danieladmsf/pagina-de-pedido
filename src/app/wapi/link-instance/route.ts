...
export async function POST(request: Request) {
  return withAuth(request, async (user) => {
    try {
      ...
      // O ID do servidor próprio vai em maiúsculas; o da W-API segue como veio.
      const idDigitado = String(body.wapiInstanceId || '').trim();
      const proprio = ehInstanciaWuzapi(idDigitado);
      const wapiInstanceId = proprio ? idDigitado.toUpperCase() : idDigitado;
      ...
      try {
        ...
        const painel = proprio ? 'do servidor proprio' : 'da W-API';
