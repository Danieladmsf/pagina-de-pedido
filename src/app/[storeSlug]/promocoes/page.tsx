...
export async function generateMetadata({ params }: { params: Promise<{ storeSlug: string }> }): Promise<Metadata> {
  ...
  if (!storeId) return { title: 'Promoções' };
  ...
  return {
    title: `Promoções | ${storeName}`,
    description: `Ofertas por tempo limitado no ${storeName}. Aproveite!`,
    ...(logoUrl ? { icons: { icon: logoUrl, shortcut: logoUrl, apple: logoUrl } } : {}),
  };
}

export default async function PromocoesPage({ params }: { params: Promise<{ storeSlug: string }> }) {
  const { storeSlug } = await params;
