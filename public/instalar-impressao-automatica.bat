...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $ProgressPreference='SilentlyContinue'; [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; $arch = if ($env:PROCESSOR_ARCHITECTURE -eq 'ARM64') {'arm64'} else {'x86_64'}; try { $rel = Invoke-RestMethod -Uri 'https://api.github.com/repos/qzind/tray/releases/latest' -Headers @{ 'User-Agent'='polaris-pdv' }; $asset = $rel.assets | Where-Object { $_.name -like ('*' + $arch + '.exe') } | Select-Object -First 1; if (-not $asset) { Write-Host 'Instalador nao encontrado para a arquitetura' $arch; exit 1 }; $out = Join-Path $env:TEMP $asset.name; Write-Host ('Baixando ' + $asset.name + ' (aguarde)...'); Invoke-WebRequest -Uri $asset.browser_download_url -OutFile $out; Write-Host 'Download concluido. Instalando QZ Tray em modo silencioso (pode levar ate 1 minuto)...'; $p = Start-Process -FilePath $out -ArgumentList '/S' -Wait -PassThru; Write-Host ('Instalacao finalizada (codigo ' + $p.ExitCode + ').'); exit $p.ExitCode } catch { Write-Host ('Falha: ' + $_.Exception.Message); exit 1 }"
